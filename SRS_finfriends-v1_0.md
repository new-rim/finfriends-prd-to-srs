...
> **설계 문서로부터의 역류 반영** — 설계 과정에서 드러난 항목 중 **요구사항·판정 규칙에 해당하는 것**을 본 판본에 반영했다: §1.5(인과 루프 R1·B1 — DESIGN §5.1~5.2) · §6.1 결정 대기 이벤트 2종(DESIGN §3.5 · OP-1) · §6.3 규칙 13 및 §9.4.3 KPI-16(병합 회고의 체류 지표 — DESIGN §7.3 · OP-7) · §9.1 ⑤ 및 §9.4.2(계측 건강성 미달 시 판정 보류 — DESIGN §7.5). 설계 수단에 해당하는 항목은 반영하지 않고 설계 문서에 둔다.
...
### 1.5 성장 루프와 불신 루프 — 요구사항이 서로를 부르는 구조

> §1.4가 **순서**(무엇 다음에 무엇이 오는가)를 그렸다면, 본 절은 **되돌이**(무엇이 무엇을 다시 부르는가)를 그린다. 요구사항 몇 건이 왜 낱개가 아니라 **한 세트로만 성립하는지**가 여기서 갈린다.
>
> 인과 루프 전체 4건은 `DESIGN_finfriends-v1_0.md` §5에 있으며, 본 절에는 그중 **요구사항의 존재 이유와 배분을 결정하는 2건**만 옮겼다. 나머지 2건(B2 계획 카드 루프 · B3 별의 사용처)은 각각 §11 **ADR-002 · ADR-003의 「대가」 항목**에 이미 기록되어 있으므로 중복해 싣지 않는다.

#### R1 — 성장 루프 *(선언 ①이 살아 있을 때)*

```mermaid
flowchart LR
    A["아동의 실천 1건<br/><small>REQ-FUNC-002 · 008 · 012</small>"] -->|"+"| B["⭐ 지급<br/><small>REQ-FUNC-004</small>"]
    B -->|"+"| C["나무 조건 충족"]
    C -->|"+"| D["나무 단계 상승<br/><small>REQ-FUNC-001</small>"]
    D -->|"+"| E["보호자가 변화를 읽음<br/><small>REQ-FUNC-009</small>"]
    E -->|"+"| F["보호자의 승인 · 미션 등록 활발"]
    F -->|"+"| A
    B -->|"+"| G["아바타 아이템 획득<br/><small>REQ-FUNC-005</small>"]
    G -->|"+"| H["아동의 다음 실천 동기"]
    H -->|"+"| A
...
> 화살표의 `+`는 *"이것이 늘면 저것도 늘어난다"*, `−`는 *"이것이 늘면 저것은 줄어든다"* 를 뜻한다. **R**은 스스로 커지는 되돌이(강화), **B**는 스스로 멈추는 되돌이(균형)다.

**이 루프에서 결정된 것 세 가지**

1. **입구가 「실천 1건」이다** — 실천이 0이면 뒤의 화살표가 **전부** 멈춘다. 북극성을 WPA 하나로 둔 이유(ADR-001)와 나무 승급 조건에 실천을 넣은 이유(ADR-006)가 같은 지점에서 나온다. §1.4의 *"③이 0이면 ④가 멈춘다"* 가 루프로 보면 이 그림이다.
2. **고리가 두 개다** — 보호자를 거쳐 돌아오는 **바깥 고리**(⭐ → 나무 → 보호자가 읽음 → 승인·미션 등록)와, 아동 안에서 닫히는 **안쪽 고리**(⭐ → 아바타 아이템 → 다음 실천 동기)다.
3. **안쪽 고리는 보호자를 거치지 않는다** — 실천 3경로 중 보호자 승인을 요구하는 것은 **미션 경로뿐**이고, 소비 회고(계획 준수 ⭐1 · REQ-FUNC-008)와 위시리스트 달성(REQ-FUNC-012)은 승인 없이 ⭐가 확정된다. 그래서 보호자의 반응이 느린 구간에도 안쪽 고리는 계속 돈다. 미션 경로의 지연은 **⭐ 소급 지급 100% 불변**(REQ-NF-007)이 흡수한다 — **소급이 실패하면 두 고리가 동시에 끊긴다.**

#### R2 · B1 — 전달 루프와 불신 루프 *(선언 ②가 걸린 지점)*

> 같은 기계가 **전달이 되면 R2로, 안 되면 B1로** 돈다. 두 루프를 한 그림에 둔 이유는 **분기점이 하나**이기 때문이다 — 보호자가 *"무엇을 했는지"* 를 읽어 내는가.

```mermaid
flowchart LR
    subgraph POS["R2 · 전달이 성공할 때 — 선순환"]
        P1["나무에 실천 근거 기본 노출<br/><small>REQ-FUNC-001</small>"] -->|"+"| P2["보호자가 '무엇을 했는지' 이해"]
        P2 -->|"+"| P3["앱에 대한 신뢰"]
        P3 -->|"+"| P4["주 1회 확인 습관"]
        P4 -->|"+"| P5["승인 지연 감소"]
    ...
    subgraph NEG["B1 · 전달이 실패할 때 — 자기강화 하강 루프"]
    ...
    FIX3["⭐ 소급 지급 100%<br/>REQ-NF-007"] -.->|"차단"| N6
...
**R2는 REQ-FUNC-001의 「실천 근거 기본 노출」에서 시작한다** — 무엇을 했는지가 읽히면 신뢰가 확인 습관을 만들고, 확인 습관이 승인 지연을 줄여, 아동의 실천이 즉시 인정된다. 이것이 선언 ②가 성립한 상태다.

**B1은 그 반대 방향으로 도는 같은 기계다** — *"아이가 멈춘다 → 보호자가 이유를 모른다 → 확인을 덜 한다 → 승인이 늦어진다 → 아이의 실천이 인정되지 않는다 → 더 멈춘다"* 의 **자기강화 하강 루프**다. §8.1이 기록한 H1형 보호자의 이탈 트리거 — *"나무 2주 이상 정체 → 자녀 탓이 아니라 앱 불신으로 조용히 이탈"* — 이 **N3**에 해당한다.

| 끊는 지점 | 요구사항 | 이 요구사항이 없으면 |
| --- | --- | --- |
| **N2** 원인을 모름 | **REQ-FUNC-001** — 미충족 조건을 조건 단위로 전부 표시 | 정체가 「앱이 멈췄다」로 읽힌다 |
| **N3** 오귀인 | **REQ-FUNC-010** — 「승인 대기 N건」 표시 | 보호자 자신의 지연이 아동의 미실천으로 보인다 |
| **N6** 인정 끊김 | **REQ-NF-007** — ⭐ 소급 지급 100% 불변 | 보호자의 지연이 아동의 실패로 기록된다 |
| **N1** 정체 진입 | **REQ-FUNC-011** — 3일 미접속 알림 + 멈춘 지점 표시 | 루프가 시작된 사실 자체를 아무도 모른다 |

> **네 건은 낱개로 뺄 수 없다** — 각각이 루프의 **서로 다른 지점**을 끊으므로, 하나만 넣으면 남은 지점에서 루프가 다시 닫힌다. §13.2가 R1 배분 근거로 적은 *"Should 5건 중 어느 하나를 빼도 「인정」 또는 「탐지」 경로가 끊긴다"* 가 이 구조를 가리킨다. **범위 축소 논의에서 이 표의 4행은 함께 움직여야 한다.**
...
**결정 대기 중인 이벤트 2종** — 아래 2종은 §9.4.4가 특정한 계측 공백을 메우는 것이며, **KPI-04(계획 카드 작성률)가 곧 REQ-FUNC-008의 생존 조건(AC-4.4)**이므로 추가되지 않으면 해당 요구사항의 검증 자체가 성립하지 않는다. 추가는 위 표(현재 10종)와 §6.2.8 `EventType`을 **함께** 개정하는 결정이므로, 확정 전까지 아래 2행은 **채택되지 않은 상태**로 둔다. 필드 사양은 `DESIGN_finfriends-v1_0.md` §3.5에 준비되어 있으나 적재 코드는 넣지 않는다.

| 인터페이스 구분 | 방향 | 명칭 | 필수 필드 | 이 이벤트로 열리는 지표 |
| --- | --- | --- | --- | --- |
| **인앱 이벤트** *(보류)* | 적재 | `plan_card_created` | `child_id` · `author`(child · parent) · `category_code` · `limit_amount` · `client_ts` | **KPI-04 분자** · 작성자 분포 — 보호자 기기 작성 비중(CON-DEV-01)의 유일한 검증 원천 |
| **인앱 이벤트** *(보류)* | 적재 | `payment_settled` | `child_id` · `amount` · `merchant_category` · `matched_plan_card_id` · `client_ts` | **KPI-04 분모** · AC-4.2 매칭 정확도의 원천 |

> **미채택 시의 처리** — AC-4.2 · AC-4.4를 **「판정 불가」로 기록**하고 릴리스 게이트(§9.5)의 통과 근거로 쓰지 않는다.
...
13. **병합 회고의 지표 처리**: 미완 회고가 큐 3건을 초과해 「요약 회고」로 병합된 건(ACE-5.2)은 **개별 체류 시간이 존재하지 않으므로 KPI-16(회고 체류 중위)의 분모에서 제외**하고 **병합 건수를 함께 표기**한다. 제외하지 않으면 병합이 잦은 주에 체류 중위가 실제보다 길게 나온다 — 회고가 형식적 클릭으로 퇴화하는지를 보는 지표가 병합 때문에 좋아 보이는 경로를 차단한다
...
**⑤ 계측 건강성 미달 시 판정** — 인앱 계측에서 파생하는 지표는 값 자체보다 **그 값을 믿을 수 있는지**가 먼저 갈린다.

| 계측 건강성(§9.4.6) | 판정 |
| --- | --- |
| **H1 이벤트 유실률 ≤ 0.5%** | 정상 판정 — ①의 3구간을 적용한다 |
| **H1 이벤트 유실률 > 0.5%** | 해당 기간의 행동 지표를 **판정에 쓰지 않는다.** 「계측 신뢰 미달」로 기록하고 §9.5 릴리스 게이트의 통과 근거로 삼지 않으며, 유실 원인을 특정한 뒤 재산출한다 |

> **값의 하락과 계측의 하락은 구분되어야 판정이 성립한다** — 유실률을 모르는 상태의 WPA 하락은 아동이 실천하지 않은 것인지 이벤트가 들어오지 않은 것인지 말할 수 없다. 이 규칙은 **PASS를 막는 방향으로만** 작동한다 — 계측이 불건전한 구간의 FAIL을 PASS로 되돌리지 않는다.
...
| 계측 건강성 H1(이벤트 유실률)이 **0.5%를 초과**한 주차 | **판정에 쓰지 않는다** — 하락이 실천 감소인지 이벤트 유실인지 구분할 수 없다. 값은 산출하되 「계측 신뢰 미달」을 병기하고 §9.1 ⑤에 따라 처리한다 |
...
| KPI | 세는 대상(분자) | 기준 모집단(분모) | 원천 · 필드 | 카운트 규칙 | 주기 |
| --- | --- | --- | --- | --- | --- |
| **KPI-16** 회고 체류 중위 | — | 회고 화면 진입 전건 *(병합 건 제외)* | `retro_viewed.dwell_ms` p50 · 1초 미만 비율 · `queue_state` | **갈래별로 분리 집계**한다(`plan_met` true/false) — 합산하면 갈래 B의 이탈이 갈래 A에 가려진다. **「요약 회고」로 병합된 건은 개별 체류가 존재하지 않으므로 분모에서 제외하고 병합 건수를 병기**한다(§6.3 규칙 13 · ACE-5.2) | 주간 |
