...
> 139건의 상세 표에 들어가기 전에 **어떤 Epic이 무엇을 책임지는가**를 먼저 본다. 개발 Epic의 번호(E1~E20)는 **§13 빌드 순서를 따르며**, 태스크 번호도 같은 순서로 매겨져 있다.

### 1.1 개발·인프라 Epic — 20구획 · 116건
...
| **E5** Auth & Consent | `FR-030~FR-034` | 5 | 1/4/0 | B1 | 보호자 전용 인증 · 동의 게이트 4겹 판정 · 캐시 금지 |
| **E6** Guardian Onboarding | `FR-035~FR-039` | 5 | 1/4/0 | B1 | 5단계 세션 분할·재개 · 법정대리인 동의 · 카드 신청 실패 보존 |
| **E7** Star Ledger & Learning | `FR-040~FR-045` | 6 | 1/5/0 | B2 | 별 지급 엔진(트리거 8종·경로 분리) · 4영역 학습과 퀴즈 |
| **E8** Practice & Mission | `FR-046~FR-050` | 5 | 1/4/0 | B3 | 미션 루프 · 승인 4건 원자 트랜잭션 · 소급 귀속 · 일괄 승인 |
| **E9** Wishlist & Avatar | `FR-051~FR-054` | 4 | 0/4/0 | B3 | 위시리스트 단계 보상 · 아바타 교환(차감도 같은 원장 경로) |
| **E10** Partner & Spending | `FR-055~FR-063` | 9 | 2/7/0 | B4 | 제휴사 웹훅 수신 규약 · 결제↔계획 매칭 · 카드 상태·해지 |
| **E11** Retrospective | `FR-064~FR-066` | 3 | 1/2/0 | B5 | 회고 문장 비복원 배정 · 두 갈래 판정 · 미완 큐와 병합 |
| **E12** Batch Infrastructure | `FR-067~FR-069` | 3 | 0/3/0 | B0 | 배치 하트비트 · `pg_net` 진입점 · 등록 목록 대조 |
| **E13** pg_cron 배치 | `FR-070~FR-079` | 10 | 5/5/0 | B5 | DB 내부 집계 9종 + 파티션 선행 생성 — 실행 시간 제한 없음 (X-5) |
| **E14** Notification | `FR-080~FR-084` | 5 | 2/3/0 | B5 | Web Push → 인앱 배너 → SMS 폴백 · 시도/전달 분리 집계 (X-3) |
| **E15** Vercel Cron 배치 | `FR-085~FR-089` | 5 | 1/4/0 | B5 | 외부 호출이 필요한 배치 4종 — 알림 발송 · 프로브 · 원가 · DLQ |
| **E16** Growth Delivery | `FR-090~FR-093` | 4 | 1/3/0 | B5 | 성장 나무 · 월간 숲 · 소비 내역 — 배치 산출값을 읽기만 한다 |
| **E17** PWA & Offline | `FR-094~FR-097` | 4 | 2/2/0 | B3 | 설치성 확보 · IndexedDB 큐 · iOS 포그라운드 flush (X-4) |
| **E18** Observability & Ops | `FR-098~FR-100` | 3 | 1/2/0 | B6 병행 | 로그 드레인 · 규제 즉시 알림 30분 SLA · 배치 침묵 감지 |
| **E19** AI Ops Tool | `FR-101~FR-106` | 6 | 1/5/0 | 조건부 | 운영자 전용 AI — 아동·보호자 런타임에서 배제 (X-7) |
| **E20** Verification Harness | `FR-107~FR-116` | 10 | 3/6/1 | 검증 | 릴리스 게이트 RG-T1~T4의 근거가 되는 검증 설비 |
