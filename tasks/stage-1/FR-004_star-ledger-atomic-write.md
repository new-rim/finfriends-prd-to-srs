...
about: Stage 1 · 개정 3.0 FR-004 (개정 2.0 FR-014 흡수)
...
- **왜 단독 이슈인가:** **별이 걸린 전 기능**(FR-013 지급 · FR-015 미션 · FR-017 보상 · FR-021 회고)이 이 위에 선다. 합치면 밀렸을 때 무엇이 밀렸는지 알 수 없다.
...
- **Blocks (직접 4건):** FR-006(Action 골격) · FR-013(grantStar) · FR-024(정산 배치) · FR-036(동시성 부하 테스트)
- **간접 30건** — 별이 걸린 기능(FR-015 미션 · FR-017 보상 · FR-021 회고)은 **FR-013를 거쳐** 이 이슈에 매달린다
- **차단 항목:** 없음 — 외부 종속이 없는 순수 DB 작업이다

## ⚠️ 임계 경로 주의

**「별이 걸린 전 기능」이 이 이슈 위에 선다.** 개정 3.0 §7이 지목한 지점이며, 여기서 설계를 바꾸면 FR-013 · 017 · 020 · 027이 함께 다시 열린다. **부하 검증은 FR-036가 정식으로 지지만, 이 이슈에서 1차 확인을 마치고 넘긴다.**
