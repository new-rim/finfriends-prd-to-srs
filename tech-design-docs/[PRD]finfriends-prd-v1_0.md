...
- **판본** — v0.1 → v0.2 → **v1.0**(2026-08-24 확정 판본). v0.2는 측정 가능성 결함 11건을 해소했고, **v1.0은 결정 정합성을 맞추고 ADR·의존성·원가·보안 임계치를 채웠습니다.** 변경 요약은 [부록 E](#부록-e-변경-내역)
- **문서 지위 — 결정은 확정, 수치는 실측 대기.** §4 기능 우선순위 · §5-1 규제 상수 · §7-1 범위 · §7-4 ADR 8건은 **확정된 결정**이며 그대로 구현 대상입니다. §1-2·§1-3의 **기준선은 실측 전**이고, 각 목표값은 §8-4 순서에 따라 확정합니다 — 실측 전 수치를 **달성한 성과로 인용하지 않습니다**(§9-2).

**등급 표기 (VPS 승계)** — `[A]` 학술·공공통계 · `[관측]` 팀 직접 확인 · `[추론]` 해석·모델값 · `[검증 대기]` 실험에 연결된 미검증 명제 · `[설계 확정]` v13 확정 사양 · `✍️[합성 가설]` 페르소나·여정·Pain · `🧪[모의]` 모의 응답(**Evidence 아님**)
...
> 2. **성능·가용성은 「AC 역산 상한」으로 확정했습니다**(§5-2) — 이 값을 넘으면 §3의 수용 기준이 성립하지 않으므로 **테스트 대상은 이미 확정**입니다. 실측 후 확정 SLO로 교체하는 것은 **상한을 조이는 절차**이지 미정 상태가 아닙니다.
...
> 6. 🆕 **결정의 이유는 [§7-4 ADR](#-7-4-adr--제품-구조-결정-기록) 8건에 모여 있습니다** — 무엇을 정했는지가 아니라 **왜 그렇게 정했고, 무엇을 포기했고, 무엇으로 틀렸음을 알게 되는지**를 각 결정마다 적었습니다.
...
|:-:|---|---|---|:-:|:-:|:-:|
| ★ **선별** | **C1(성장 증거 부재)** | 누적 숫자가 성장의 증거가 되지 못한다 | 나무 5초 노출 후 **행동 변화 1문장 회상 성공률 < 50%** · 부모 주 1회 확인 시간 **> 8분** | **인터뷰 T3·Q0-2에서 실측** | ✍️[합성 가설] | ✅ |
| ★ **선별** | **C2(실천 공백 → 나무 정체)** | 실천 칸이 비어 4개 나무가 전부 정체한다 | **가입 후 7일 내 첫 실천 인정률 < 40%** · 월말 4칸 중 2칸 이상 성장 가정 **< 30%** | **프로토타입 1주차 실측** | ✍️[합성 가설] | ✅ |
...
| ◇ **보조** | **C3(정체 원인 미설명)** | 2주 정체를 부모가 아이의 능력 문제로 해석한다 | 정체 발생 시 **원인 표시 열람률 < 40%** · 오귀인 발화 비율 **> 40%** | **인터뷰 Q1-9에서 실측** | ✍️[합성 가설] | ✅ |
...
| △ **감시** | **C6(회고 형식화)** | 🔄 **범위 축소** — ⭐는 **계획 준수 시에만** 지급(v13 §5). 남은 위험은 **계획을 지킨 날에 회고를 읽지 않고 누르는 것** | **동일 회고 문장 재노출률 > 50%**(7일 창) · **계획 준수 건의** 회고 화면 체류 **중위 < 2초** | **프로토타입 1~2주차 실측** | [설계 확정]+[추론] | ✅ |
| △ **감시** | **A1(「불리기」 학습 장벽)** | 「불리기」가 학습 단계부터 막힌 칸이 된다 | 4주제 중 **「불리기」 학습 완주율이 최저이며 차상위와 격차 > 20%p** | **MVP 1개월차 실측** | ✍️[합성 가설] | 📋 **관측만** |
...
| # | Outcome | 지표 | 기준선 *(확보 시점)* | **목표** | 측정 주기 | 측정 도구 |
|:-:|---|---|:-:|:-:|:-:|---|
| O1 | 변화가 한 문장으로 읽힌다 | 나무 5초 노출 후 1문장 회상 성공률 | **인터뷰 T3 시점 확보** *(§8-4 ⑤)* | **≥ 6/8** | 인터뷰 회차 | 인터뷰 T3 (n=8) |
| O2 | 확인이 짧아진다 | 부모 주 1회 확인 소요시간 | **인터뷰 Q0-2 재연 계측** *(자기 앱을 열게 하고 실측 · §8-3)* | **3분** | 주간 | 인앱 세션 시간 + 인터뷰 Q0-2 |
| O3 | 부모–자녀 대화가 생긴다 | 돈 이야기 월 횟수 | **인터뷰 자기보고** *(§8-4 ⑤)* | **월 2회** | 월간 | 🔴 **자기보고 단독 — 인앱 관측 불가.** 교차 검증 수단이 없어 **보조 지표로만** 사용하고 게이트 기준으로 쓰지 않음 |
| O4 | 첫 실천이 반영된다 | 가입 후 7일 내 첫 실천 인정률 | **프로토타입 1~2주차** *(§8-4 ④)* | **≥ 60%** | 주간 | 인앱 이벤트 |
| O5 | 네 칸이 고르게 자란다 | 월말 4칸 중 2칸 이상 성장 가정 비율 | **MVP 1개월차** | **≥ 50%** | 월간 | 인앱 집계 |
| O6 | 실천 경로가 실제로 쓰인다 | 🔄 **실천 트리거 3종**(미션 승인·소비 회고·위시리스트) **중 1종 이상 도달 아동 비율** *(WPA-v1 범위와 동일)* | **프로토타입 2주차** | **≥ 70%** | 월간 | 인앱 이벤트 |
...
| O10 | 멈춘 이유를 알 수 있다 | 정체 원인 표시 열람률 · 실천 근거 열람률 | **MVP 1개월차** *(정체 발생 이후 산출 가능)* | **≥ 60%** | 월간 | 인앱 화면 로그 |
| O13 | 아이가 멈춘 것을 부모가 곧 안다 | 아이 루프 정지 → 부모 인지 일수 | **대체재 기준선 = 최대 한 달** *(전환 결정 시점에 인지 · §2-2 10단계)* | **≤ 3일** | 주간 | F11(3일 미접속 알림) 알림 발송·열람 로그 |
| O14 | 부모 루프가 돈다 | 다음 달 충전율 · 월간 숲 익월 재방문율 | **MVP 2개월차** *(익월 지표라 1개월차엔 산출 불가)* | **≥ 70% / ≥ 50%** | 월간 | 인앱 집계 |
...
| 항목 | 값 |
|---|---|
| **기준선** | **프로토타입 2주차 산출** *(§8-4 ④)* |
| **목표** | **≥ 5/8 가정** *(β 클로즈드 · n=8)* → 일반 공개 후 **≥ 55%** |
...
| 축 | 지표 | 기준선 *(확보 시점)* | 목표 | 주기 | 측정 경로 |
|---|---|:-:|:-:|:-:|---|
| 선언 ②(그 성장이 보인다) *(전달)* | 나무 5초 회상 · 부모 확인 시간 | 인터뷰 8슬롯 | **≥ 6/8** · 3분 | 인터뷰 회차 · 주간 | 인터뷰 T3 rubric · `forest_view_opened.dwell_ms` |
| 선언 ②(그 성장이 보인다) *(신뢰)* | 실천 근거 열람률 · 정체 원인 표시 열람률 | MVP 1개월차 | ≥ 60% | 월간 | `tree_view_opened.evidence_expanded` · `.stall_reason_shown` |
| J3(끊김 방지) *(인정)* | 승인 지연 시 ⭐ 소급 지급 성공률 | — *(불변 100%)* | **100%** | 주간 | `approval_state_changed(state=backfilled)` |
| J3(끊김 방지) *(탐지)* | 아이 정지 → 부모 인지 일수 | 대체재 = 최대 한 달 | ≤ 3일 | 주간 | `inactivity_notified` |
| 데이터 타당성 | 회고 화면 체류 중위값 · 동일 문장 재노출률 | 프로토타입 1~2주차 | ≥ 3초 · **≤ 2/8** | 주간 | `retro_viewed.dwell_ms` p50 · `sentence_id` 중복률 |
| 지속 | 아이 3일/7일 재접속률 · 익월 숲 재방문율 | 프로토타입 2주차 · MVP 2개월차 | ≥ 65%/50% · ≥ 50% | 주간·월간 | 세션 로그 · `forest_view_opened` |
| 관계 *(보조)* | 부모–자녀 돈 대화 월 횟수 | 인터뷰 자기보고 | 월 2회 | 월간 | 🔴 **자기보고 단독 — 인앱 관측 불가.** 게이트 기준으로 쓰지 않음 |
| **수익** 🔴 | **카드 연결률 · 아동 1인당 월 결제액** | MVP 1개월차 *(관측 시작)* | **검증과제 ⑥ 후 역산 확정** | 월간 | **확정 방법** — 제휴사 수수료율 확인 후 **`월 결제액 × 수수료율 ≥ C_child`**(§5-4 B1)를 만족하는 최소값. 그때까지 **원가 쪽(B1~B4)만 선행 관리** |
...
📑 **이 절이 인용한 VPS 절과 등급** — §1-3(Pain 8건 ✍️합성 가설) · §1-5(신호 0건 [추론]) · §3-1(O1~O14 [검증 대기]) · §3-2(순서 역전 [추론]) · §8-0(v13 사양 [설계 확정]) · §8-3(컷라인 근거) · §9(무료 확정 · SOM 무효)
...
| # | Given / When / Then | 측정 도구 · 표본 |
|:-:|---|---|
| **AC1** | **Given** 여정 6단계, 가입 7일 이내 아동 / **When** 실천 트리거 **3종**(미션 승인·소비 회고·위시리스트) 중 1종을 최초로 완료한다 / **Then** **⭐ 지급 + 해당 나무 진행도 갱신이 동일 세션 내 반영** · **가입 7일 내 첫 실천 인정률 ≥ 60%** | 인앱 이벤트 로그 · 프로토타입 1~2주 (Core 3 · Adjacent 2) |
...
| 🆕 **AC-E2** | **Given** 계획한 업종과 **다른 업종에서만** 결제 (금액은 계획 이내) / **When** 대조 화면을 만든다 / **Then** **⭐1을 지급하되 회고 문장을 「업종 다름」 갈래로 분기**하고, `plan_met=true` · `category_met=false`로 기록한다 *(MVP 기본값 — **금액 단독 판정**)* | 단위 테스트 + `retro_viewed.category_met` |
...
📑 **인용 VPS 절과 등급** — §2-2(Job 정의) · §3-1(O 지표 [검증 대기]) · §1-2·§1-3(Pain·여정 ✍️합성 가설) · §8-0(사양 [설계 확정]) · §2-1(🧪모의 발화)
...
### 🆕 4-1-1. 선행 의존성 · 스프린트 환산

> 1 스프린트 = **2주**. 난이도 5→2sp · 4→1.5sp · 3→1sp · 2→0.5sp로 환산합니다.
> **Could 이상 전 기능이 1스프린트 안에 들어가지는 않습니다** — 초과 4건은 아래에 **분할 단위를 명시**해 스프린트 경계를 맞춥니다.

| F | 우선순위 | **선행 의존성** *(이것이 없으면 착수 불가)* | sp | 1sp 내 | **초과분 분할 단위** |
|---|:-:|---|:-:|:-:|---|
| **F1** 성장 나무 | Must | F3(학습 이수) · F4(실천 판정 결과) | 1.5 | ✖ | ① 4칸 단계 렌더 → ② **정체 원인 표시**(US-3) |
| **F2** 미션 루프 | Must | F7(부모 계정·동의) · F4 | 1 | ✅ | — |
| **F3** 학습 커리큘럼 | Must | **콘텐츠 4주제 원고**(외부 선행) | 1 | ✅ | — |
| **F4** 별 지급 엔진 | Must | F7(동의 게이트) | 1 | ✅ | — |
| **F5** 아바타·옷장 | Must | F4(별 잔액) · **3D 에셋**(종수·벌수 미결 — §7-3) | 1.5 | ✖ | ① 아바타 1종 + 옷 3벌 → ② 종수 확장 *(스펙 확정 후)* |
| **F7** 부모 온보딩·동의 | Must | **제휴사 카드 발급 API**(§6-3) · 본인인증 | 1.5 | ✖ | ① 동의·계정(1~3단계) → ② **카드 신청**(4~5단계) |
| **F8** 계획 카드·대조 | Must | **제휴사 결제내역 API + 업종 코드 상세도**(§6-3) · F4 | 2 | ✖ | ① 계획 카드 CRUD → ② **결제 매칭**(E7 ≥ 90%) → ③ 두 갈래 회고 |
| **F9** 월간 숲 | Must | F1(나무 상태) · F4 · F8(소비 집계) | 1 | ✅ | — |
| **F6** 아이 온보딩 | Should | F3 · F4 · F5 | 0.5 | ✅ | — |
| **F10** ⭐ 소급 지급 | Should | F2 · F4(원장 `idempotency_key`) | 0.5 | ✅ | — |
| **F11** 3일 미접속 알림 | Should | F7(알림 시간대 설정) · 푸시 인프라 | 0.5 | ✅ | — |
| **F12** 위시리스트 | Should | F4 | 0.5 | ✅ | — |
| **F13** 소비 내역 | Should | F8(업종 집계) · 제휴사 결제내역 API | 0.5 | ✅ | — |
| **F15** 예적금 실천 | Could | 🔴 **P-20 법률 검토**(검증과제 ⑤) — **블로커** | 1 | ✅ | 착수 조건: 법률 검토 통과 |
| **F16** 카드 없는 체험 | Could | F3 · F7 온보딩 분기 | 1 | ✅ | — |
| **F17** 별의 옷장 외 목적지 | Could | 🔴 **P-21 현금 분리선 재검토** — **블로커** | 1 | ✅ | 착수 조건: 분리선 재확인 |
| **F18** 기록 이전 | Won't | 외부 앱 데이터 이관 협의 — **경로 없음** | 2 | ✖ | *(Won't — 분할 계획 없음)* |

> **크리티컬 패스** — `F7 → F4 → {F1, F2, F8}`. **F7·F4가 늦어지면 Must 8건 중 6건이 함께 밀립니다.**
> 🔴 **F8은 유일하게 외부 계약에 착수가 걸린 Must**입니다(업종 코드 상세도). 계약 확인이 지연되면 **① 계획 카드 CRUD만 먼저 내고 ②를 다음 스프린트로** 넘깁니다.
...
| **P-19** | 🔄 **적용 대상 아님 — 위치정보를 수집하지 않습니다**(F14·F19 삭제 · ADR-002). 요건을 회피한 것이 아니라 **요건이 발생하지 않는 구조**입니다 | 스키마·서버 로그에 좌표 필드 **부재** 검증(일 1회 스캔) · 패킷 검사 — **위치 기능 재도입 시 이 행을 상수로 복원** |
...
> **이 값들은 §3의 수용 기준에서 역산했습니다**(ADR-007) — AC가 성립하지 않는 값은 어떤 경우에도 허용될 수 없기 때문입니다. v0.1이 전부 미정이라 **테스트 대상이 존재하지 않았던 문제**를, 확정치를 지어내지 않으면서 해소한 방식입니다.
...
> **이 값의 지위 — 확정된 상한이며, 테스트 대상입니다.** 각 값은 §3의 수용 기준에서 역산했으므로 **초과하면 해당 AC가 성립하지 않습니다.** 실측 후 이 절을 확정 SLO로 교체하되, 교체는 **상한을 조이는 방향으로만** 합니다 — 실측치가 상한보다 나쁘면 값을 늦추는 것이 아니라 **재설정 트리거를 발동**합니다.

### 5-3. 아동 개인정보 · 보안

- **위치정보 미수집**(F14·F19 삭제 · ADR-002) · **얼굴 이미지 미수집**(P-13) · 학습·실천 데이터는 **아동 식별정보와 분리 저장**
- 아이 계정은 **부모 계정에 종속** — 독립 로그인·외부 공유·친구 기능 **없음**(v13 §11 범위 제외)
- 별·저금통 간 **전환 경로 자체를 코드에 두지 않음**(P-21: 별↔저금통 분리) — 기능 플래그로 막는 방식 금지

#### 🆕 보안 임계치 · 모니터링 항목

> 위 세 줄은 **원칙**이고, 아래가 **측정 대상**입니다. 전부 **허용 오차 0**입니다 — 아동 데이터라 「낮게 유지」가 아니라 「없음」이 기준입니다.

| # | 항목 | 계산식 / 소스 | 임계치 | 점검 주기 | 알림 수신자 |
|:-:|---|---|:-:|:-:|---|
| **S1** | 서버 스키마·로그에 **좌표 필드 유입** | 스키마 스캔 + 로그 정규식 | **0건** | 일 1회 + 배포 시 | 개발 온콜 + 정책 담당 |
| **S2** | **얼굴 이미지 필드** 존재(P-13) | 스키마 스캔 | **0건** | 배포 시 | 개발 온콜 |
| **S3** | **아동 식별정보 ↔ 학습·실천 데이터 결합 조회** | 쿼리 감사 로그 | **0건** | 일 1회 | 정책 담당 |
| **S4** | **별↔저금통 전환 코드 경로** 존재(P-21) | 정적 분석 — 전환 함수·API 심볼 검색 | **0건** | 배포 시(CI 게이트) | 개발 담당 |
| **S5** | 아동 계정 **독립 로그인 시도** | 인증 로그(부모 세션 부재) | **0건** | 일 1회 | 개발 온콜 |
| **S6** | 동의 미완 상태 아이 진입(P-05) | `consent_gate_blocked` | **0건** | 실시간 | 개발 온콜 + 정책 담당 |

> **S4는 CI 게이트입니다** — 전환 경로가 코드에 나타나면 **빌드를 실패**시킵니다. 「기능 플래그로 막기」를 금지한 이유가 여기서 강제됩니다.

### 5-4. 비용

| 항목 | 내용 |
|---|---|
| **수익 전제** | **구독 매출 0원 — 부모·아이 모두 무료** **[설계 확정]** · ADR-005. 결제 수수료 + 금융사 제휴 **2중 구조** |
| **주요 원가** | 제휴사 수수료(협상 대상) · 3D 에셋 제작(**동물 종수 × 옷 벌수 미결**) · 클라우드 · 본인인증 |
| 🔴 **리스크** | **수수료 종속** — 결제 수수료가 사실상 유일한 수익원인데 그 몫을 제휴사와 나눔. 실제 수수료율·최소 물량은 **검증과제 ⑥에서 확정** |

#### 🆕 원가 임계치 · 모니터링 항목

> **손익분기 항등식** — `아동 1인당 월 결제액 × 수수료율 ≥ 아동 1인당 월 원가(C_child)`
> 우변(원가)은 **우리가 통제하고 지금 계측할 수 있으며**, 좌변의 수수료율만 검증과제 ⑥ 대상입니다. 따라서 **수수료율을 몰라도 원가 상한 관리는 즉시 시작합니다.**

| # | 항목 | 계산식 / 소스 | 임계치 | 주기 | 수신자 | 초과 시 대응 |
|:-:|---|---|:-:|:-:|---|---|
| **B1** | **아동 1인당 월 원가** `C_child` | (클라우드 + 본인인증 상각 + 제휴 고정비) ÷ 활성 아동 수 | **BEP 이하** — 수수료율 확정 전까지 **관측 + 추세만** | 월간 | 사업 담당 | 3개월 연속 상승 → 원가 구조 재설계 |
| **B2** | **클라우드 월 비용** | 청구서 + 태그별 집계 | 전월 대비 **+30%** | 월간 *(주간 예측)* | 개발 담당 | 원인 특정 전까지 신규 인스턴스 증설 보류 |
| **B3** | **본인인증 호출** | 호출 건수 ÷ 온보딩 완료 건수 | **≤ 1.2회/건** | 주간 | 개발 담당 | 재시도 남용 — 온보딩 4단계 UX 점검 |
| **B4** | **3D 에셋 제작비** *(일회성)* | 종수 × 벌수 × 단가 | 🔴 **스펙 확정 전 제작 착수 금지** | 게이트 | 제품 담당 | 종수·벌수 미결(§7-3) 해소가 **선행 조건** |
| **B5** | **결제 수수료 수입** | 월 결제액 × 수수료율 | 검증과제 ⑥ 확정 후 **B1 대비 비율**로 감시 | 월간 | 사업 담당 | BEP 미달 2개월 → 제휴 조건 재협상 |

> **B4가 게이트인 이유** — 3D 에셋은 **선불 일회성 지출**이라 사후 조정이 불가능합니다. 종수·벌수가 미결인 채 제작에 들어가면 **되돌릴 수 없는 원가**가 확정됩니다.
...
| 성능 | 화면별 p95 | §5-2 측정 방법 | AC 역산 상한 초과 **3일 연속** | 개발 담당 | 3일 내 원인 특정 | 릴리즈 중단 |
| 🆕 🔴 보안 | S1~S6 *(§5-3)* | 스키마·로그 스캔 · 정적 분석 | **> 0건 즉시** | 개발 온콜 + 정책 담당 | **30분 내 확인 · 즉시 경로 차단** | S4는 **CI 빌드 실패** · 2시간 미해결 → 서비스 일시 중단 |
| 🆕 비용 | B1~B3 *(§5-4)* | 청구서 · 호출 로그 | B1 3개월 연속 상승 · B2 **+30%** · B3 **> 1.2** | 사업 담당 + 개발 담당 | 월간 리뷰 *(B2는 주간)* | BEP 미달 2개월 → 제휴 조건 재협상 |
...
| **아동 계정** | id · 보호자_id · 생년(만 나이) · 기기 유형(전용폰/키즈워치/공유/없음) | 기기 유형은 **기능 판정에 쓰지 않습니다**(F14·F19 삭제) — 알림 채널 선택(F11)과 **모집 스크리너 분류**에만 사용 |
...
- **위치정보를 일절 수집하지 않습니다**(F14·F19 삭제 · ADR-002) → **P-19 적용 대상 아님.** 좌표 스키마를 **두지 않는 것을 넘어, 위치 판정 경로 자체가 없습니다**
...
| 계획↔실제 대조 *(F8)* | 계획 카드(업종·금액) · 결제 승인 업종 코드 | 매칭 결과 · 계획 준수 여부 | 🔴 **업종 코드 상세도가 매칭 정확도(US-4 AC2 ≥ 90%)를 좌우** — 계약 시 확인 |
...
| **Out** *(영구 — 삭제 확정)* | 📍 위치 알림(F14) · 🚦 소비 사전 개입 2차 트리거(F19) — **2026-08-21 삭제**(§4-2 · ADR-002). 로드맵 항목이 아니라 **폐기된 기능**이며, 사전 수단은 **F8 계획 카드**가 대신합니다 |
| **Out** *(이번 릴리즈 — 후속 판본 대상)* | 예적금 실천(F15 · P-20 법률 검토 선행) · 카드 없는 체험(F16) · 별의 옷장 외 목적지(F17 · P-21 재검토 선행) · 기록 이전(F18) |
...
| **R5** 🔴 | **수익식의 좌변이 외부에 있음** — 무료 확정(ADR-005)으로 SOM Level 3 무효, 수수료율은 제휴사 종속 | 수수료율 확정 전까지 **수익 목표를 세울 수 없음** | 검증과제 ⑥ · KPI를 **카드 연결률·월 결제액**으로 재정의 · 🆕 **우변(원가 C_child)은 §5-4 B1~B4로 즉시 관리 착수** |
...
| 유형 | 내용 | 확정 방법 | 상태 |
|---|---|---|---|
| ~~의존성~~ | ~~아이 기기에서 알림 수신 가능 여부~~ | — | ✅ **해소** *(2026-08-21)* — F14·F19 삭제로 알림 수신이 전제에서 빠짐(R2 · ADR-002). 실기기 검토 **불필요** |
| 🔴 **의존성** | **제휴사 수수료율 · 최소 물량 조건 · SLA · 업종 코드 상세도** | 제휴사 문의(검증과제 ⑥) | **제휴 계약 체결 전 확정** — ADR-004·005·008이 함께 걸림 |
| 🔴 **의존성** | **예적금 「가입 연결」의 중개업 해당 여부**(P-20: 예적금 가입 중개 회피) | 법률 검토(검증과제 ⑤) | **F15 착수 전 확정** — 미확정 시 F15는 v0.3으로 이월 |
| ~~의존성~~ | ~~위치정보법 만 14세 미만 동의 요건(P-19)~~ | — | ✅ **해소** — 위치정보 미수집으로 **요건 자체가 발생하지 않음**(§5-1 · §6-2) |
| **가정** | 부모가 **금융 집중을 원한다** | 인터뷰 H1-3 (검증과제 ①) | **E6로 판정** — 8슬롯 완료 시점 *(🧪 모의는 (b))* |
| **가정** | 🔄 아이·부모가 **가기 전에 계획 카드를 적는다** *(구: 「위치 알림에서 참는다」 — F14 삭제로 교체)* | 인앱 실측(검증과제 ②) | **E3로 판정** — MVP 4주 시점 |
| **가정** | 부모가 **이자를 실제로 준다** | 검증과제 ③ *(10만원 × 연 20% = 월 1,700원)* | **E10에 편입** — 3개월 종단 |
| **가정** | AOS·DOS 순위가 우선순위로 유효 | — | **[추론]** — 합성 가설 위의 판단값, **순위 비교용** |
| **미결(사양)** 🔴 | **동물 종수 · 옷 벌수** | 팀 결정 | **F5 착수 전 확정** — §5-4 **B4 제작 착수 게이트** *(선불 지출이라 사후 조정 불가)* |
| **미결(사양)** 🔴 | **출석체크 조건** | 팀 결정 | **F4 스펙 확정 전** — H1·H2 신호가 정반대(아래) |
| **미결(사양)** | 나무 성장 단계 수·조건 수치 · 매달 조건 상승 여부 · 이자 주기 · 승인 지연 지급 타이밍 | 팀 결정 | **프로토타입 착수 전**(§8-4 ④) — v13 §14 승계 |
...
> **ADR(Architecture Decision Record)** — 무엇을 정했는지가 아니라 **왜 그렇게 정했고 무엇을 포기했는지**를 남깁니다.
> 이 절의 8건은 **되돌리려면 다른 결정까지 함께 흔들리는 것**만 골랐습니다. 각 ADR에는 **그 결정이 서 있는 비즈니스 가설과 이를 판정할 실험**을 붙였습니다 — 결정이 틀렸음을 **무엇으로 알게 되는가**가 기록의 핵심입니다.

| ADR | 결정 | 상태 |
|:-:|---|:-:|
| **001** | 북극성 KPI를 **WPA 하나**로 둔다 | ✅ 확정 |
| **002** | F14·F19를 삭제하고 사전 개입을 **F8 계획 카드**로 옮긴다 | ✅ 확정 |
| **003** | 별은 **차감형·초기화 없음**, 별↔저금통 **완전 분리** | ✅ 확정 |
| **004** | 선불업을 직접 등록하지 않고 **제휴사에 위탁**한다(B안) | ✅ 확정 |
| **005** | 부모·아이 **모두 무료** — 수익은 **결제 수수료 + 금융사 제휴** | ✅ 확정 |
| **006** | **실천 0건이면 나무가 승급하지 않는다** | ✅ 확정 |
| **007** | 성능·가용성을 **AC 역산 상한**으로 확정하고 SLO 확정을 미룬다 | ✅ 확정 |
| **008** | 계획↔실제 판정을 **금액 단독**으로 한다 | ✅ 확정 |

---

#### ADR-001 — 북극성 KPI를 WPA 하나로 둔다

| | |
|---|---|
| **맥락** | 선언 ①(성장이 일어난다)과 ②(그 성장이 보인다) 중 어느 쪽을 북극성으로 둘지가 로드맵 우선순위를 통째로 바꿉니다 |
| **결정** | **WPA(주간 실천 인정 아동 비율)** 단일. 분자는 실천 트리거 3종, 분모는 활성 아동(동의 완료 · 계정 7일 경과 · 28일 내 세션) |
| **기각한 대안** | ① **나무 5초 회상률** — 인터뷰 회차 지표라 상시 계측 불가 ② **다음 달 충전율** — 월 1회 지연 지표 ③ **아이 7일 재접속률** — 출석 ⭐1만으로 올라 **활동량 지표로 퇴화** |
| **서 있는 가설** | *"아이가 실천하지 않으면 부모 화면은 거짓말을 하지 않고 **비어 있을 뿐**이고, 그 빈 화면이 「우리 애가 안 하는구나」로 읽힌다"* — 즉 **①이 죽으면 ②도 죽는다** |
| **판정 실험** | **E4**(7일 내 첫 실천) · **E10**(행동 전이 3개월 종단) |
| **뒤집히는 조건** | E10에서 **실천은 늘고 3지표가 불변**이면 「실천 인정」이 성장의 대리 지표가 아니라는 뜻 → 🔴 **선언 ①의 「성장」 정의부터 재작성** |
| **대가** | 부모 쪽 전달 품질(선언 ②)이 **보조 KPI로 내려갑니다.** 전달이 나빠도 WPA는 올라갈 수 있어, §1-3 보조 KPI로 **별도 감시**합니다 |

#### ADR-002 — F14·F19를 삭제하고 사전 개입을 F8 계획 카드로 옮긴다

| | |
|---|---|
| **맥락** | 선별 Pain C5(사전 개입 사각지대)의 대응 기능이 위치 알림(F14)·자동 트리거(F19)였는데, **선결 조건 4개 중 3개가 미해결**이었습니다(기기 전제 · 체류시간 근거 · P-19 구현 범위 · 온라인 결제 수단) |
| **결정** | 자동 트리거를 만들지 않고, 사전 개입을 **「쓰기 전에 적는 행위」** 로 옮깁니다. **위치정보를 일절 쓰지 않습니다** |
| **기각한 대안** | ① F14 Won't 유지 ② 경량 트리거 분할 ③ Must 승격 — **셋 다 기기 전제와 사각지대가 그대로 남습니다.** 특히 ②는 해결해도 소용없습니다: **3분 트리거가 못 잡는 소비가 곧 C5**였기 때문 |
| **서 있는 가설** | *"아이·부모는 결제 전에 계획을 적을 의사가 있고, 적은 것과 실제를 맞춰보는 것만으로 행동이 바뀐다"* |
| **판정 실험** | **E3**(계획 카드 작성률 ≥ 50% · MVP 4주) — **이 결정의 단일 생존 조건** |
| **뒤집히는 조건** | E3가 **< 30%(FAIL)** 이면 사전 절반이 비고 대조할 것이 없어 **사후 절반까지 무너집니다** → 계획 작성 UX 재설계 · C5 발생률 재산출. 자동 발동 재도입은 **이 판정 이후에만** 논의합니다 |
| **대가** | ① **자동 발동이 사라졌습니다** — 사각지대가 없어진 게 아니라 「3분 미만 소비」에서 **「계획을 적지 않은 소비」로 옮겨갔습니다** ② **C10·C12가 완전 미대응**(대응 기능 0개) ③ **대외 문구에서 지오펜싱 서술을 전부 내려야 합니다** |
| **파급** | P-19 적용 대상에서 빠짐(§5-1) · R2 해소(§7-2) · E7 교체(§8-2) · 기기 유형이 기능 판정에서 빠짐(§6-1) |

#### ADR-003 — 별은 차감형·초기화 없음, 별↔저금통 완전 분리

| | |
|---|---|
| **맥락** | 별을 현금성으로 만들면 **선불전자지급수단 규제**에 걸리고, 주기마다 초기화하면 아이가 **모은 것을 잃는 경험**이 됩니다 |
| **결정** | 별은 **차감형**(사용 시 감소) · **초기화 없음** · **별↔저금통 전환 경로를 코드에 두지 않음**. ❌ 현금 충전 ❌ 현금 환급 ❌ 양도·거래 |
| **기각한 대안** | 기능 플래그로 전환 경로를 막는 방식 — **플래그는 켜질 수 있습니다.** 경로 자체를 두지 않아야 규제 검증이 성립합니다 |
| **서 있는 가설** | *"별은 양(활동량), 나무는 질(내재동기)을 예측한다"* — 인센티브와 성장 지표를 **분리해야 출석 ⭐1이 실천 지표를 밀어올리지 않는다** |
| **판정 실험** | **§9-1 [G] 검증** — WPA 분자에서 트리거 1·2·3 제외 후 **출석률과 WPA 상관 r ≤ 0.3** |
| **뒤집히는 조건** | r > 0.3이면 제외 규칙만으로는 분리가 안 된다는 뜻 → 별 지급 트리거 재설계 |
| **대가** | 별의 쓸 곳이 **옷장 하나뿐**이라 모으기형 아이의 루프 회수가 약합니다(F17이 Could이자 P-21 재검토 대기) |
| **강제 수단** | §5-3 **S4 — 전환 경로가 코드에 나타나면 CI 빌드 실패** |

#### ADR-004 — 선불업을 직접 등록하지 않고 제휴사에 위탁한다 (B안)

| | |
|---|---|
| **맥락** | 아동 카드 발급에는 선불전자지급수단 발행업 등록과 **선불충전금 100% 별도관리**가 필요합니다 |
| **결정** | 발행·관리·가맹점망을 **제휴사**가, 브랜드·앱·4칸 학습·실천 판정을 **핀프렌즈**가 담당합니다 |
| **기각한 대안** | 직접 등록 — 자본·인가 기간이 MVP 일정을 초과합니다 |
| **대가** | 🔴 **한도·업종 제한을 우리가 정할 수 없습니다**(P-17·P-18) · 🔴 **가용성 SLA가 제휴사보다 높을 수 없습니다**(§5-2) · 🔴 **수수료를 제휴사와 나눕니다**(R5) |
| **판정 실험** | **검증과제 ⑥** — 수수료율 · 최소 물량 · SLA · **업종 코드 상세도** |
| **뒤집히는 조건** | 업종 코드 상세도가 낮으면 **F8 매칭 정확도(E7 ≥ 90%)가 성립하지 않습니다** → 업종 대조 포기 · 금액 단독 판정으로 축소(ADR-008) |

#### ADR-005 — 부모·아이 모두 무료, 수익은 결제 수수료 + 금융사 제휴

| | |
|---|---|
| **맥락** | 최대 경쟁자(아이부자 · 78만명)가 이미 0원이고, 대체재인 계좌·카드형이 **83.9%** 를 점유합니다 — **가격 경쟁이 봉쇄돼 있습니다**(R7) |
| **결정** | 부모·아이 **모두 무료**. 수익은 **결제 수수료 + 금융사 제휴 2중 구조** |
| **파급** | **SOM Level 3(구독 전제 · 440가구 × 월 4,900원) 산출식이 무효**가 됩니다 · 수익 KPI를 **「유료 전환율」이 아니라 「카드 연결률 · 월 결제액」** 으로 재정의합니다 |
| **서 있는 가설** | *"무료여도 카드·자녀 연결 의향이 생긴다"* |
| **판정 실험** | **모집 Non-user 1**(H10형 · 무료 대체재 사용 중) · **검증과제 ⑥**(수수료율) |
| **뒤집히는 조건** | `월 결제액 × 수수료율 < C_child`가 2개월 지속되면 **BEP 미달** → 제휴 조건 재협상(§5-4 B5) |
| **대가** | 🔴 **수수료 종속** — 유일한 수익원의 몫을 제휴사와 나누며, 우리가 요율을 정하지 못합니다 |

#### ADR-006 — 실천 0건이면 나무가 승급하지 않는다

| | |
|---|---|
| **맥락** | 퍼핀·아이부자는 **승급에 필요한 최소 실천 횟수가 0회**라, 학습만으로 화면이 자랍니다 — 그 화면은 **행동 변화의 증거가 아닙니다**(C1) |
| **결정** | 나무 승급 조건에 **학습·퀴즈·실천을 모두** 넣고, **실천 0건이면 학습·퀴즈를 초과 충족해도 승급하지 않습니다** |
| **기각한 대안** | 실천을 가중치로만 반영 — 학습량으로 상쇄돼 **「실천 0건 최고 단계」가 가능**해집니다 |
| **서 있는 가설** | *"「가르치는 것」과 「행동을 바꾸는 것」은 다른 문제다"* — 지식 +0.33 SD vs 행동 +0.07 SD **[A]** |
| **판정 실험** | **US-2 AC2 · AC-E3 단위 테스트**(실천 0건 승급 0건) · **E10**(행동 전이 종단) |
| **뒤집히는 조건** | E10에서 실천 증가가 3지표 개선으로 이어지지 않으면 **승급 조건의 「실천」 정의를 재설계**(ADR-001과 함께 흔들림) |
| **대가** | 🔴 **초기 화면이 오래 비어 보입니다.** 특히 학습 진도가 느린 H2형은 **벌기·불리기 칸이 오래 비어** 「고르지 않게」 보입니다 → US-1 AC-E1(빈 화면 안내)·§4-4로 대응 |

#### ADR-007 — 성능·가용성을 AC 역산 상한으로 확정하고 SLO 확정을 미룬다

| | |
|---|---|
| **맥락** | 실측 없이 SLO를 확정하면 근거가 없고, 비워 두면 **테스트 대상이 존재하지 않습니다**(v0.1이 전부 `⬜ 미정`이었던 문제) |
| **결정** | **§3의 수용 기준에서 역산한 상한**을 확정 값으로 씁니다 — *"이 값을 넘으면 해당 AC가 성립하지 않는다"* 가 판정 근거입니다 |
| **기각한 대안** | ① 업계 관행값 차용 — 우리 AC와 무관해 초과해도 무엇이 깨지는지 말할 수 없습니다 ② 미정 유지 — 테스트가 불가능합니다 |
| **예외 — 협상 불가** | **별 지급·차감 정합성 오류율 0%** 와 **⭐ 소급 지급 성공률 100%** 는 역산이 아니라 **불변**입니다. 아이가 모은 것을 잃는 경험이라 허용 오차가 없습니다 |
| **갱신 규칙** | 실측 후 **상한을 조이는 방향으로만** 교체합니다. 실측치가 상한보다 나쁘면 **값을 늦추지 않고 재설정 트리거를 발동**합니다 |
| **대가** | 가용성 **99.0%** 는 제휴사 SLA 미확인 상태의 보수적 하한이라, 확인 후 **min(우리 목표, 제휴사 SLA)** 로 낮아질 수 있습니다(ADR-004 종속) |

#### ADR-008 — 계획↔실제 판정을 금액 단독으로 한다

| | |
|---|---|
| **맥락** | 계획한 업종과 다른 업종에서만 결제했고 **금액은 계획 이내**인 경우, 「지켰다」로 볼 것인가가 열려 있었습니다(구 US-4 AC-E2 · 팀 결정 대기) |
| **결정** | **⭐는 금액 기준으로만 판정**합니다. 업종 불일치는 **회고 문장을 「업종 다름」 갈래로 분기**하고 `category_met=false`로 기록하되, **⭐를 막지 않습니다** |
| **기각한 대안** | 업종 일치를 ⭐ 조건에 포함 — ① 업종 코드 상세도가 **제휴사 종속**이라 우리가 통제할 수 없고(ADR-004) ② 매칭 오류가 곧 **⭐ 미지급**이 되어 아이가 **시스템 오류로 보상을 잃습니다**(ADR-003의 「모은 것을 잃지 않는다」와 충돌) |
| **서 있는 가설** | *"금액을 지키는 것이 1차 행동 변화이고, 업종 정확도는 데이터 품질 문제이지 보상 조건이 아니다"* |
| **판정 실험** | **E7**(업종 매칭 정확도 ≥ 90%) · `retro_viewed.category_met` 분포 |
| **뒤집히는 조건** | 매칭 정확도가 **≥ 90%로 안정**되고 `category_met=false` 비율이 유의하게 높으면, **업종 일치를 ⭐ 조건으로 승격**하는 안을 v0.3에서 재검토합니다 |
| **대가** | *"적은 대로 썼는가"* 를 **금액으로만** 판정하므로, **계획한 곳과 다른 데서 쓴 소비가 「지킴」으로 집계**됩니다 — 이 비율을 `category_met`으로 **상시 관측**합니다 |
...
> ⚠️ **H2형은 「전용 스마트폰 있음」과 「키즈워치만」을 각각 1가정씩** 확보합니다 — R2가 해소돼 **기능 판정용이 아니라**, 계획 카드를 **누가(아이/부모) 어느 기기에서 적는가**를 관찰하기 위한 대조입니다(E3 해석용).
...
| **E7** | 🔄 **계획↔실제 업종 매칭이 정확하다** *(구: 「아이 기기에 알림이 도달한다」 — F14·F19 삭제로 폐기)* | 카드 승인 업종 코드 ↔ 계획 카드 수동 대조 **50건** | 매칭 정확도 · 오매칭 유형 | **≥ 90%** *(US-4 AC2)* | 70~89% | **< 70%** → 업종 대조를 포기하고 **금액 단독 판정**으로 축소 · US-4 AC-E2 재결정 |
...
| **② 개입 시점** | **결제 전에 기록되는 수단 개수** | 0개 | 0개 | 0개 | **1개** — F8 계획 카드 *(자동 발동은 0개)* | 기능 목록 대조 |
...
flowchart LR
    ...
    B --> C["③ 업종 갈래 회고 검수<br/>금액 단독 판정 확정<br/>(US-4 AC-E2 · ADR-008)"]
    ...
    style C fill:#e6f4e6,stroke:#2d8a2d,stroke-width:2px
```
...
| # | 쓰지 말 것 | 대신 |
|:-:|---|---|
| 1 | 기준선·목표 수치를 **성과**로 인용 | **실측 전 목표값** — *"실측 후 확정"* |
| 2 | *"퍼핀은 확인 기능이 없다"* | *"확인은 됩니다. 그게 성장인지 알 수 없는 것이 문제입니다."* |
| 3 | AOS·DOS를 *"검증됨"·"수학적으로 완벽"* | *"합성 가설 위의 분석자 판단값이며 순위 비교용"* |
| 4 | **「소비 순간 자동 개입」을 제공 기능·로드맵**으로 | *"가기 전에 적고, 쓴 뒤에 맞춰봅니다"* — 자동 트리거는 **폐기**(F19 삭제 · ADR-002)이며 로드맵에도 없음 |
...
| **2** | **기능 17개 · 우선순위(Must·Should·Could·Won't) = Must 8 / Should 5 / Could 3 / Won't 1** — v13·VPS의 F 번호를 승계하되 **F14·F19를 삭제**(19 → 17 · §4-2 · ADR-002)하고 나머지 컷라인은 유지 |
...
| **6** | ✅ **최상위 결정 종결 — F14·F19 삭제, 사전 수단을 F8 계획 카드로 이관**(§4-2 · ADR-002). 남은 것은 *"대응이 없다"* 가 아니라 **"자동 발동이 없다"** 이며, 이 상태를 §1-4·§4-2·§7-2·§8-3에 네 번 남김 |
| **7** | **선결 순서 = 결제 매칭 검증 → 계획 카드 작성률 실측 → 업종 일치 ⭐ 판정 결정 → 프로토타입 → 인터뷰 → v0.3**(§8-4) — 앞의 둘은 문항이 아니라 **계측**이며, 인터뷰를 먼저 하면 답이 무의미해짐 |
...
| **12** | 🆕 **ADR 8건 신설**(§7-4) — 각 결정에 **서 있는 가설 · 판정 실험 · 뒤집히는 조건 · 대가**를 붙여, 결정이 틀렸음을 **무엇으로 알게 되는가**까지 기록 |
| **13** | 🆕 **선행 의존성 · 스프린트 환산**(§4-1-1) — 크리티컬 패스 `F7 → F4 → {F1, F2, F8}` 확정. **1스프린트 초과 4건은 분할 단위를 명시** |
| **14** | 🆕 **원가·보안 임계치 신설**(§5-3 S1~S6 · §5-4 B1~B5) — 손익분기 항등식의 **우변(원가)은 수수료율을 몰라도 즉시 관리**하고, 보안 6항목은 **전부 허용 오차 0**(S4는 CI 빌드 실패) |
| **15** | 🆕 **F14·F19 삭제를 문서 전체에 전파** — §6-1·§6-3·§7-1·§7-3·§8-1·§8-2·§8-3·§9-2와 최종 요약에 남아 있던 **낡은 「결정 대기」 표기 11곳을 확정 상태로 교체** |

---

## 부록 E. 변경 내역

### E-1. v0.2 → v1.0 *(2026-08-24)*

> **v0.2는 「어떻게 측정하는가」를 확정했고, v1.0은 「결정이 서로 어긋나지 않는가」와 「왜 그렇게 정했는가」를 채웠습니다.**

**① 결정 정합성 — F14·F19 삭제가 전파되지 않아 생긴 충돌 11곳 해소**

| 위치 | v0.2에 남아 있던 것 | v1.0 |
|---|---|---|
| §6-1 아동 계정 | 기기 유형이 **F14·F19 판정에 쓰임** | 기능 판정에 **쓰지 않음** — 알림 채널·모집 분류 전용 |
| §6-2 · §6-3 | 온디바이스 **위치 판정 인터페이스** 존치 | 행 삭제 → **F8 계획↔실제 대조**로 교체 |
| §5-1 P-19 | 위치정보 **옵트인 상수**로 존치 | **적용 대상 아님** — 요건이 발생하지 않는 구조 |
| §7-1 Out | *"F19 — **결정 대기**"* | **삭제 확정** · Out을 「영구」/「이번 릴리즈」로 분리 |
| §7-3 의존성 2건 | 알림 수신 · P-19 요건 **⬜ 미확인 블로커** | 둘 다 **✅ 해소** |
| §7-3 가정 | *"아이가 **위치 알림에서** 참는다"* | *"**가기 전에 계획 카드를 적는다**"* + **E3 판정** |
| §8-1 | *"R2 판정용"* 기기 대조 | R2 해소 반영 — **E3 해석용** 관찰로 목적 교체 |
| §8-2 E7 | *"아이 기기에 알림이 도달한다"* | **업종 매칭 정확도 ≥ 90%** 로 교체 |
| §8-3 ② | 표 **0개** ↔ 주석 *"0 vs 0이 아니다"* 모순 | **1개**(F8) — 표·주석 정합 |
| §9-2 #4 | *"로드맵 1순위(F19)"* | 자동 트리거는 **폐기 · 로드맵에도 없음** |
| 최종 요약 #2·#6·#7 | **19개 / Must 8·Should 6·Could 3·Won't 2** · *"F19 배치 결정 대기"* · 낡은 선결 순서 | **17개 / 8·5·3·1** · **결정 종결** · §8-4 순서로 교체 |

**② 표기 — 결정에 붙은 「가설」 경고 제거**

| # | v0.2 | v1.0 | 왜 |
|:-:|---|---|---|
| **1** | 헤더 *"모든 목표 수치는 **[가설]** … 어느 수치도 성과로 인용할 수 없습니다"* | **"결정은 확정, 수치는 실측 대기"** | 결정(§4·§5-1·§7-1·§7-4)과 미실측 수치(§1-2·§1-3)를 **분리**. 인용 금지는 §9-2에 그대로 유지 |
| **2** | 지표 표 컬럼 **「목표 [가설]」** | **「목표」** | 목표는 **약속**이지 가설이 아닙니다. 검증 대상은 목표가 아니라 **기준선**입니다 |
| **3** | §5-2 *"위 값은 **[가설]이며 SLO가 아닙니다**"* | *"**확정된 상한이며, 테스트 대상입니다**"* | AC 역산값은 추측이 아니라 **도출된 제약**입니다(ADR-007) |
| **4** | §7-3 가정 3건 **🔴 [가설]** | **E6 / E3 / E10 판정** + 판정 시점 | 「미검증」보다 **「무엇으로 언제 판정되는가」** 가 실행 정보입니다 |
| **5** | 등급 표기 `[가설]` 미검증 | `[검증 대기]` **실험에 연결된** 미검증 명제 | 실험에 연결되지 않은 명제는 이제 없습니다(§9-1) |

**③ 신설 — 패스 조건 미달분 보완**

| 항목 | v0.2 상태 | v1.0 |
|---|---|---|
| **기능 의존성 · 스프린트** | 난이도만 있고 **의존성·스프린트 없음** | 🆕 **§4-1-1** — 선행 의존성 · sp 환산 · **1sp 초과 4건 분할 단위** · 크리티컬 패스 |
| **ADR** | **0건** — 결정 이유가 본문에 흩어짐 | 🆕 **§7-4 ADR 8건** — 결정 · 기각한 대안 · **서 있는 가설 · 판정 실험 · 뒤집히는 조건 · 대가** |
| **보안 임계치** | §5-3이 **원칙 3줄** — 측정 대상 없음 | 🆕 **S1~S6** 전부 허용 오차 **0** · **S4는 CI 빌드 실패** · §5-5에 보안 계층 추가 |
| **원가 임계치** | §5-4가 **항목 나열** — 임계치·모니터링 없음 | 🆕 **B1~B5** + **손익분기 항등식** — 수수료율을 몰라도 **우변(원가)은 즉시 관리** · **B4는 제작 착수 게이트** |
| **기준선 확보 시점** | 대부분 **⬜** | 전 지표에 **확보 시점 명시**(프로토타입 N주차 / MVP N개월차 / 인터뷰 8슬롯) |
| **US-4 AC-E2** | ⬜ **판정 미결 · 팀 결정 대기** | ✅ **금액 단독 판정 확정**(ADR-008) — 업종 불일치는 회고 갈래 분기 + `category_met` 관측 |
| **트리거 종수** | O6·US-2 AC1이 **「5종」** *(WPA-v1은 3종)* | **3종**으로 통일 — WPA-v1 범위와 일치 |

---

### E-2. v0.1 → v0.2 *(2026-08-21)*
...
**v1.0에서도 열려 있는 것 4건** *(미결이 아니라 「계측 대기」입니다 — 확보 시점이 전부 §8-4에 박혀 있습니다)*

| # | 내용 | 언제 닫히는가 |
|:-:|---|---|
| **1** | **확정 SLO** | AC 역산 상한으로 **테스트는 이미 가능**합니다(ADR-007). 프로토타입 실측 후 **상한을 조이는 방향으로만** 교체 — §8-4 ④ |
| **2** | **② 개입 시점 축의 「자동 발동」** | F8 계획 카드로 **1 vs 0**이 됐으나 **자동 발동은 0개**입니다(§8-3). 재도입은 **E3 판정 이후에만** 논의합니다 — 로드맵 항목이 아닙니다(ADR-002) |
| **3** | **기준선 실측치** | 전 지표에 **확보 시점을 명시**했습니다(§1-2). 프로토타입 1~2주차(O4·O6) → MVP 1~2개월차(O5·O10·O14) → 인터뷰 8슬롯(O1·O2·O3) 순으로 닫힙니다 — §8-4 ④⑤ |
| **4** | **제휴 조건**(수수료율 · SLA · 업종 코드 상세도) | 검증과제 ⑥. **ADR-004·005·008이 함께 걸려 있는 유일한 외부 종속**이며, 원가 쪽 대응(§5-4 B1~B4)은 **이미 착수 가능**합니다 |
