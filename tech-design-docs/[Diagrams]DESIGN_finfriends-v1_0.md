...
>
> **예외 — 의도적으로 공유하는 2건.** §5.1 R1 성장 루프와 §5.2 B1 불신 루프는 **요구사항의 존재 이유를 결정하는 그림**이라 SRS §1.5에도 실려 있다. SRS 쪽은 요구사항 배분 관점(어느 요구사항이 루프의 어느 지점을 끊는가), 본 문서 쪽은 설계 관점(그 지점을 어느 컴포넌트·시퀀스가 구현하는가)으로 읽는다. **두 곳의 화살표 구조는 같아야 하며, 한쪽만 고치지 않는다.**
...
> **여기서 제안해 SRS로 승격된 규칙** — 병합된 회고는 개별 체류 시간이 존재하지 않으므로 **KPI-16 분모에서 제외하고 병합 건수를 병기**한다. 넣지 않으면 병합이 많은 주에 체류 중위가 실제보다 길게 보인다. 본 규칙은 **SRS §6.3 규칙 13 · SRS §9.4.3 KPI-16 카운트 규칙으로 반영**되었다(OP-7 해소).
...
| --- | --- | --- | --- | --- |
| **OP-1** 🔴 | `plan_card_created` · `payment_settled` 이벤트 추가 | KPI-04 산출 불가 → **REQ-FUNC-008의 생존 조건(AC-4.4)이 측정 불가** | SRS §6.1(10종) · SRS §6.2.8 `EventType` 개정 — **SRS §6.1에 「결정 대기 중인 이벤트 2종」으로 게재되어 채택 대기 중** | §3.5에 스키마를 준비해 두고 적재 코드를 넣지 않는다. 미채택 시 AC-4.2 · AC-4.4는 「판정 불가」로 기록 |
...
| **OP-7** ✅ *(해소)* | 병합된 회고의 체류 지표 처리 | KPI-16 분모 정의에 병합 건이 없었다 | — **결정 완료** | **SRS §6.3 규칙 13 · SRS §9.4.3 KPI-16에 반영됨.** 분모 제외 + 병합 건수 병기로 구현한다 |
